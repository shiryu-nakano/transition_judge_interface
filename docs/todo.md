...
**やること(0524)**
- [X] ~~*最低限のnodeの実装*~~ [2026-05-24 17:55]
  - [X] ~~*規約を守ること*~~ [2026-05-24 17:55]
    - 規約については後述する
- 動作確認
...
ここまででこのノードが持つ機能：
- 外界のセンサー情報をサブスクライブして内部の変数に保持できるようになった
- 外界のセンサー情報を受け取って、componentが持つjudge関数に入力するためのJudgeInput構造体を作成する機能がある→内部で呼び出して作成している
- timerCallbackで、逐次状態遷移の判定を行っている（＝judge(input)を読んでいる）

ここから
* [ ] corの仕組みを作成する。つまりjudgeinputを受け取って、handlerが処理するor次に流す
  * [ ] かんたんな例から
  * [ ] 一つだけのhandlerで、stopかそのままかを決めてみる
    * [ ] 仮想環境で、直進した先に障害物が１つだけあるというケースを作成する
    * [ ] 障害物が見つかったら、stop　nodeをonにする
      * これはmotion primitivesにあるので、これを呼び出しておく
  * 
* [ ] chain自体はstrategy patternとして実装する
  * [ ] area_idで異なるchainに入力できるようにする



# 一番はじめにかんたんなロジックで動くものをハードに実装してしまうか
# まずstrategyとcorの仕組みを、
